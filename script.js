...
function handleCRFormSubmit(event) {
    ...
    const crDescription = formData.get("crDescription");
    
    if (editingCRId) {
        // Update existing CR member
        updateCRMember(editingCRId, crName, crDescription);
    } else {
        // Add new CR member
        addCRMember(crName, crDescription);
    }
}

function addCRMember(name, description) {
...
function updateCRMember(id, name, description) {
...
function editCRMember(id) {
    ...
    document.getElementById("crDescription").value = cr.description || "";
...
function resetCRForm() {
    ...
    document.getElementById("crImagePreview").innerHTML = "";
    editingCRId = null;
    document.getElementById("updateCRBtn").classList.remove("hidden");
    document.getElementById("cancelCRBtn").classList.add("hidden");
        ...
        html += `
            <div class="cr-member-card">
                ...
                <div class="cr-member-info">
                    <h4>${cr.name}</h4>
                    ${cr.description ? `<div class="cr-member-description">${cr.description}</div>` : ''}
        ...
        html += `
            <div class="candidate-card" onclick="selectCandidate('${cr.name}')">
                ...
                <div class="candidate-info">
                    <h4>${cr.name}</h4>
                    ${cr.description ? `<div class="candidate-description">${cr.description}</div>` : ''}
                </div>
            </div>
        `;
    });
...
function showResults() {
    ...
    crMembers.forEach(cr => {
        ...
        li.textContent = `${cr.name}: ${votes[cr.name] || 0} vote(s)`;
        resultsList.appendChild(li);
    });
    
    // Show NOTA results
    const notaLi = document.createElement("li");
    notaLi.textContent = `Nota: ${votes["Nota"] || 0} vote(s)`;
    resultsList.appendChild(notaLi);
}
